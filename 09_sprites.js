...
function _bridgeBuild(R){
  ...
  // The gatehouse TOWERS, one on each side of both ends (user prefers these to the pillar posts).
  // `y` is the base line; drawBridge sinks the sprite into it and runs the balustrade through and
  // behind it, which is what stops it reading as a separate object sat on the bridge.
...
function drawBridge(){
  ...
  // the gate towers, drawn last so they stand over the rail (that occlusion is the grounding cue)
  const tw=(typeof _bridgeTower!=='undefined'&&_bridgeTower&&_bridgeTower.naturalWidth)?_bridgeTower:null;
  const SINK=18;   // the deck and the balustrade bite the lowest course of the plinth
  if(tw) for(const a0 of P.arches){ const a={x:a0.x,y:a0.y+SINK,f:a0.f,top:a0.top};
    const w=TILE*3.1, h=w*tw.height/tw.width;
    // corruption on the far gate: a glow BEHIND the stone, never a rect over it
...
function render(){
  ...
  if(curRoom.dungeon){
    ...
    if(curRoom.npc){ const np=curRoom.npc, wob=0.55+Math.sin(t9*2.2)*0.12;
      ...
      ctx.beginPath(); ctx.ellipse(np.x,np.y+10,34,13,0,0,6.29); ctx.stroke();
